...
-- MOD START
-- ########################################
--Using a custom Tooltip Type so we can have a wider tooltip; as a bonus, we now generate the tooltip only as needed
local tipCombinedPromotion = {}
TTManager:GetTypeControlTable("TypeCombinedPromotions", tipCombinedPromotion)

function CombinedPromotionTooltip(control)
	local strEdgeHelper = " <QPBZSN> " --useful to surround strings to ensure pattern matching works correctly when match is at the beginning or end of a string; gibberish for ease of removal
	local unit = UI.GetHeadSelectedUnit()
	local rawTable = {} --raw key/value table
	local rawArray = {} --raw index/key array
	local ttStr = "" --tooltip string
	--Loop through all the promotions to find the ones we need and group together those with matching keys
	for unitPromotion in GameInfo.UnitPromotions() do
		if unit:IsHasPromotion(unitPromotion.ID) and not unit:IsTrade() then
			--if the Type ends in _1,_2,_3 etc then strip that off so we can use it as a key for grouping; or just use the Type as is
			local shortType, rank = string.match(unitPromotion.Type, "^(.+)_(%d+)$")
			local key = shortType or unitPromotion.Type
			if not rawTable[key] then
				table.insert(rawArray, key)
				rawTable[key] = {}
			end
			--take the bits we need and might as well translate the strings while we are here
			table.insert(rawTable[key], {
				Rank = rank,
				TitleText = Locale.ConvertTextKey(unitPromotion.Description), --Why is the title called a Description when the verbose description is in Help
				HelpText = Locale.ConvertTextKey(unitPromotion.Help),
			})
		end
	end
	for _, key in ipairs(rawArray) do
		local promotionTitle = rawTable[key][1].TitleText
		local promotionHelp = ""
		if #rawTable[key] == 1 then
			promotionHelp = rawTable[key][1].HelpText
		else
			local combinedTable = {}
			local combinedArray = {}
			local groupCount = #rawTable[key]
			for index, promotion in ipairs(rawTable[key]) do
				--Update the title as needed
				if groupCount < 5 then
					--if the group has less then 5 promotions then title it like: "Killer Promotion I,II,III,IV"
					if index > 1 then
						promotionTitle = promotionTitle..","..ConvertToRomanNumeral(promotion.Rank)
					end
				elseif index == groupCount then
					--if the group has 5 or more promotions then title it like: "Killer Promotion I-VII"
					promotionTitle = promotionTitle.."-"..ConvertToRomanNumeral(promotion.Rank)
				end
				--grab all the data we need out of the help
				local data = {}
				for frag, num in string.gmatch(promotion.HelpText, "(%D*)(%d*)") do
					for roman in string.gmatch(strEdgeHelper..(frag or "")..strEdgeHelper, "%A([IVXLCDM]+)%A") do --require roman numerals to have surrounding non letter characters
						if (roman or "") ~= "" then table.insert(data, roman) end
					end
					if (num or "") ~= "" then table.insert(data, tonumber(num)) end
				end
				--turn the help text into something that will be usable later
				local helpFormatted = strEdgeHelper..promotion.HelpText..strEdgeHelper				--pattern matching helpers
				helpFormatted = string.gsub(helpFormatted, "%%", "%%%%")							--escape the magical percent signs; replaces % with %% as string.format will automatically convert it back
				helpFormatted = string.gsub(helpFormatted, "%d+", "%%d")							--replace all the arabic numbers with %d
				helpFormatted = string.gsub(helpFormatted, "(%A)[IVXLCDM]+(%A)", "%1%%s%2")			--replace all the roman numerals with %s; %s being string in formatting while it is white space in pattern matching
				helpFormatted = string.gsub(helpFormatted, strEdgeHelper, "")						--remove the helpers as they have done their job
				--make a clean help for better matching of sentences when combining due to inconsistent strings; ie PROMOTION_INTERCEPTION_2 vs PROMOTION_INTERCEPTION_3
				local helpCleaned = helpFormatted
				helpCleaned = string.gsub(helpCleaned, "%[.-%]", "")								--remove all the markup code in brackets
				helpCleaned = string.gsub(helpCleaned, "%s+", "")									--strip out all the white spaces
				helpCleaned = Locale.ToUpper(helpCleaned)											--remove case sensitive; I assume this function is better then string.upper in handling unicode chars
				--combine all promotions in a group that have the same cleaned help text
				if not combinedTable[helpCleaned] then
					table.insert(combinedArray, helpCleaned)
					combinedTable[helpCleaned] = {
						Data = data,
						HelpFormatted = helpFormatted,
					}
				else
					local tbl = combinedTable[helpCleaned]
					--merge the data
					for i, d in ipairs(data) do
						if type(d) == "number" then --add the numbers
							tbl.Data[i] = tbl.Data[i] + d
						elseif type(d) == "string" then --concat the roman numerals with comma separators
							--no need to concat duplicate roman numerals, most likely caused by falsely detecting words like "I" as roman numerals
							if not string.match(strEdgeHelper..tbl.Data[i]..strEdgeHelper, "%A"..d.."%A") then
								tbl.Data[i] = tbl.Data[i]..","..d
							end
						end
					end
					if string.len(tbl.HelpFormatted) < string.len(helpFormatted) then
						--new helpFormatted has more markup and therefore should be better
						tbl.HelpFormatted = helpFormatted
					end
				end
			end
			for i, helpCleaned in ipairs(combinedArray) do
				--if not all the sentences match then place a space between each and concat them
				if i > 1 then promotionHelp = promotionHelp.." " end
				local tbl = combinedTable[helpCleaned]
				promotionHelp = promotionHelp..string.format(tbl.HelpFormatted, unpack(tbl.Data))
			end
		end
		--Time to start assembling our tooltip string
		if ttStr ~= "" then ttStr = ttStr.."[NEWLINE]" end
		ttStr = ttStr.."[COLOR_YELLOW]"..promotionTitle.."[ENDCOLOR]"
		if promotionTitle ~= promotionHelp then
			ttStr = ttStr.."  "..promotionHelp
		end
	end
	--Might as well tack on some non promotion related Defensive Bonuses
	--Terrain Modifier
	local plotMod = unit:GetPlot():DefenseModifier(unit:GetTeam(), false, false) --int = Plot:DefenseModifier(TeamID defendTeam, bool ignoreBuilding, bool help)
	if plotMod ~= 0 and unit:IsCombatUnit() and (plotMod < 0 or not unit:NoDefensiveBonus()) then
		if ttStr ~= "" then ttStr = ttStr.."[NEWLINE]" end
		local color = plotMod < 0 and "[COLOR_NEGATIVE_TEXT]" or "[COLOR_POSITIVE_TEXT]"
		ttStr = ttStr.."[COLOR_CYAN]"..Locale.ConvertTextKey("TXT_KEY_EUPANEL_TERRAIN_MODIFIER").."[ENDCOLOR]  "
		ttStr = ttStr..color..string.format("%+d", plotMod).."%[ENDCOLOR] "..Locale.ConvertTextKey("TXT_KEY_EUPANEL_DEFENSE_BONUS")
	end
	--Fortification Modifier
	local fortifyMod = unit:FortifyModifier()
	if fortifyMod > 0 then
		if ttStr ~= "" then ttStr = ttStr.."[NEWLINE]" end
		local color = fortifyMod < 0 and "[COLOR_NEGATIVE_TEXT]" or "[COLOR_POSITIVE_TEXT]" --unnecessary, but if I find a 3rd bonus like this then I might group the redundant code 
		ttStr = ttStr.."[COLOR_CYAN]"..Locale.ConvertTextKey("TXT_KEY_EUPANEL_FORTIFICATION_BONUS").."[ENDCOLOR]  "
		ttStr = ttStr..color..string.format("%+d", fortifyMod).."%[ENDCOLOR] "..Locale.ConvertTextKey("TXT_KEY_EUPANEL_DEFENSE_BONUS")
	end
	--Display the Tooltip
	tipCombinedPromotion.TooltipLabel:SetText(ttStr)
	tipCombinedPromotion.TooltipMouseover:SetHide(ttStr == "")
	tipCombinedPromotion.TooltipMouseover:DoAutoSize()
end
Controls.UnitIcon:SetToolTipCallback(CombinedPromotionTooltip)

function ConvertToRomanNumeral(arabic)
	--Completely Overkill; but there is no reason to limit the potential of a dynamic system on a run-as-needed tooltip
	--https://www.rosettacode.org/wiki/Roman_numerals/Encode#Lua
	arabic = tonumber(arabic or 0)
	local roman = ""
	local convert = (arabic < 40) and {{10,"X"},{9,"IX"},{5,"V"},{4,"IV"},{1,"I"}} or 
		{{1000,"M"},{900,"CM"},{500,"D"},{400,"CD"},{100,"C"},{90,"XC"},{50,"L"},{40,"XL"},{10,"X"},{9,"IX"},{5,"V"},{4,"IV"},{1,"I"}}
	for _, arr in ipairs(convert) do
		value, letter = unpack(arr)

print("This is the modded CityView from 'UI - City Expansion'")
...
----------------------------------------------------------------------------------------------------
--
-- Mods to support ShowInCityView entries on the following tables
--
-- Buildings, Resources and Specialists
--
-- Use SQL to add the ShowInCityView column to the required table(s), eg
--   ALTER TABLE Buildings ADD ShowInCityView INTEGER DEFAULT 1;
-- (It is recommended to place each ALTER TABLE statement in its own .sql file, in case of conflicts.)
-- Use XML/SQL to set ShowInCityView to 0 for entries you don't want to appear in the City View, eg
--   <ShowInCityView>0</ShowInCityView>
--
...
local gShowInCityViewShowSupport = {}

function ShowInCityViewShow(table, data)
	if (gShowInCityViewShowSupport[table] == nil) then
		gShowInCityViewShowSupport[table] = false
		...
		for row in DB.Query("SELECT * FROM " .. table .. " LIMIT 1") do
			for col, _ in pairs(row) do
				if (col == "ShowInCityView") then
					gShowInCityViewShowSupport[table] = true
					break
				end
			end
		end
	...
	if (gShowInCityViewShowSupport[table] == false) then
		-- The default is to show all items in the City View for tables that don't have a ShowInCityView column
		return true
	end

	-- As the ShowInCityView columns are added by altering the table with SQL, the game core doesn't know about them
	-- so we can't use data.ShowInCityView, hence we need to execute a DB.Query() to get the value directly and check it
	for row in DB.Query("SELECT ShowInCityView FROM " .. table .. " WHERE ID=" .. data.ID) do
		-- Explicit check for ShowInCityView required, as in Lua the number 0 is also "true"
		return (row.ShowInCityView == 1 or row.ShowInCityView == true)
	end
...
--
--   for building in GameInfo.Buildings() do ... end
--   for building in ShowInCityViewIterator("Buildings") do ... end
--
--   for policy in GameInfo.Buildings(string.format("PreReqTech = '%s'", techType)) do ... end
--   for policy in ShowInCityViewIterator("Buildings", string.format("PreReqTech = '%s'", techType)) do ... end
--

function ShowInCityViewIterator(table, condition)
	local next = coroutine.create(function ()
		for row in GameInfo[table](condition or "ID=ID") do
			if ShowInCityViewShow(table, row) then
				coroutine.yield(row)
			end
		end
...
--  local pSpecialistInfo = ShowInCityViewLookup("Specialists", iSpecialistID)
--
-- MUST always be followed by a check for nil
--

function ShowInCityViewLookup(table, key)
	local row = GameInfo[table][key]

	if not(ShowInCityViewShow(table, row)) then
		return nil
	end
...
local g_ForceMode = nil
function GetInterfaceMode()
  local mode = g_ForceMode or UI.GetInterfaceMode()
  return mode
end
...
function SoftInterfaceMode(mode)
  mode = g_ForceMode or mode
  UI.SetInterfaceMode(mode)
end

function ForceInterfaceMode(mode)
  g_ForceMode = mode
  UI.SetInterfaceMode(mode)
end
LuaEvents.CityViewForceBuyPlotsMode.Add(function() ForceInterfaceMode(InterfaceModeTypes.INTERFACEMODE_PURCHASE_PLOT) end)
...
--				local pSpecialistInfo = GameInfo.Specialists[building.SpecialistType];
				local pSpecialistInfo = ShowInCityViewLookup("Specialists", building.SpecialistType);
				if (pSpecialistInfo) then
...
function OnCityViewUpdate()
	...
	if (pCity ~= nil) then
		...
		local cityAutomatons = 0
		if (pCity.GetAutomatons) then
			cityAutomatons = math.floor(pCity:GetAutomatons())
		end
		Controls.CityPopulationLabel:SetText(tostring(cityPopulation + cityAutomatons));
		Controls.PeopleMeter:SetPercent( pCity:GetFood() / pCity:GrowthThreshold() );
		
		--Update suffix to use correct plurality.
		Controls.CityPopulationLabelSuffix:LocalizeAndSetText("TXT_KEY_CITYVIEW_CITIZENS_TEXT", (cityPopulation + cityAutomatons));
		
		if (pCity.GetAutomatons) then
			local cityPopulationToolTip = Locale.ConvertTextKey("TXT_KEY_CITYVIEW_CITIZENS_TT", cityPopulation, cityAutomatons)
			Controls.CityPopulationLabel:SetToolTipString(cityPopulationToolTip)
			Controls.CityPopulationLabelSuffix:SetToolTipString(cityPopulationToolTip)
		end
...
--		for pSpecialistInfo in GameInfo.Specialists() do
		for pSpecialistInfo in ShowInCityViewIterator("Specialists") do
...
--		for building in GameInfo.Buildings() do
		for building in ShowInCityViewIterator("Buildings") do
...
--				if thisBuilding.GreatWorkCount > 0 then
				if building.GreatWorkCount > 0 and pCity:GetNumSpecialistsAllowedByBuilding(building.ID) <= 0 then
...
--			local pResourceInfo = GameInfo.Resources[pCity:GetResourceDemanded()];
			local pResourceInfo = ShowInCityViewLookup("Resources", pCity:GetResourceDemanded());
			if (pResourceInfo) then
...
function OnEnterCityScreen()
	...
	SoftInterfaceMode(InterfaceModeTypes.INTERFACEMODE_SELECTION);
end
...
function UpdateWorkingHexes()
	...
	if (UI.IsCityScreenUp()) then   
		...
		g_BuyPlotButtonIM:ResetInstances();
		if GetInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_PURCHASE_PLOT then
			...
			for i = 0, pCity:GetNumCityPlots() - 1, 1 do
				...
				if (plot ~= nil) then
					...
					if (pCity:CanBuyPlotAt(plot:GetX(), plot:GetY(), false)) then
						...
						if (iPlotCost == 0) then
							controlTable.BuyPlotAnchoredButtonLabel:LocalizeAndSetText("TXT_KEY_FREE");
							Events.SerialEventHexHighlight( ToHexFromGrid( Vector2( plot:GetX(), plot:GetY() ) ), true, Vector4( 1.0, 0.0, 1.0, 1 ) );
						else
...
function OnBuyPlotClick()
	...
	SetInterfaceMode(InterfaceModeTypes.INTERFACEMODE_PURCHASE_PLOT);
	UpdateWorkingHexes();
...
function OnMouseOverHex( hexX, hexY )
	...
	if GetInterfaceMode() == InterfaceModeTypes.INTERFACEMODE_PURCHASE_PLOT then
		...
		if (city:CanBuyPlotAt( hexX, hexY, true)) then
			...
			if (iPlotCost == 0) then
				strText = "TXT_KEY_FREE";
			else
				strText = "[ICON_GOLD] " .. iPlotCost;
			end
		end
		
		Controls.BuyPlotText:LocalizeAndSetText(strText);
	end

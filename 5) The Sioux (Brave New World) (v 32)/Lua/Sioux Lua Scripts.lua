						...
						elseif pPlot:GetOwner() ~= -1 then
							...
							if (mPlayer:GetCivilizationType() ~= GameInfoTypes.CIVILIZATION_SIOUX_MOD) then
								...
								if NotificationCount >= 1 then
									...
									for cityIndex = 0, mPlayer:GetNumCities() - 1, 1 do
										local pCity = mPlayer:GetCityByID(cityIndex);
										if (pCity ~= nil) then
									...
									if (pNearestCity ~= nil) then
										local cName = Locale.ConvertTextKey(pNearestCity:GetName());
										local pAdj = Locale.ConvertTextKey(mPlayer:GetCivilizationAdjective());
										local pID = pPlayer:GetID()
										local mID = mPlayer:GetID()
										if (pPlayer:IsHuman()) and (pID == Game.GetActivePlayer()) and not (mPlayer:IsHuman()) then
											Events.GameplayAlertMessage("[COLOR_NEGATIVE_TEXT]".. pAdj .." soldiers have slaughtered [ICON_RES_BISON] Bison near ".. cName .."![ENDCOLOR]");					
										end
										if (mPlayer:IsHuman()) and not (pPlayer:IsHuman()) --[[and (mID == Game.GetActivePlayer()) --]]then
											Events.GameplayAlertMessage("[COLOR_NEGATIVE_TEXT][ICON_RES_BISON] Bison have been lost near ".. cName .."![ENDCOLOR]");					
										end
									end
...
function(iOldOwner, bIsCapital, iX, iY, iNewOwner, iPop, bConquest)
			...
			if pTargetPlot then
				if (pTargetPlot:GetImprovementType() == iTipi) then
					if ((pTargetPlot:GetOwner()) ~= (cCity:GetPreviousOwner())) then	
						if not pTargetPlot:IsImprovementPillaged() then
	...
	if cTipi >= 1 then
		local cName = Locale.ConvertTextKey(cCity:GetName());
		local pAdj = Locale.ConvertTextKey(pPlayer:GetCivilizationAdjective());
		local nID = nPlayer:GetID()
		if (nPlayer:IsHuman()) and (nID == Game.GetActivePlayer()) then
			Events.GameplayAlertMessage("[COLOR_NEGATIVE_TEXT]".. pAdj .." citizens have packed their Tipi and migrated away from ".. cName ..".[ENDCOLOR]");					
		end
	end
...
function JFD_ShowSiouxTipiPopup(bCity, iCity, ePop)
	...
	Controls.Summary:LocalizeAndSetText("Follow the [ICON_RES_BISON] Bison to new hunting grounds?[NEWLINE][NEWLINE][ICON_BULLET][COLOR_NEGATIVE_TEXT]-1 [ICON_CITIZEN] Population from " .. bCity:GetName() .. " (" .. bCity:GetPopulation() .. ")[ENDCOLOR][NEWLINE][ICON_BULLET][COLOR_POSITIVE_TEXT]+1 [ICON_CITIZEN] Population to " .. iCity:GetName() .. " (" .. iCity:GetPopulation() .. ")[ENDCOLOR]")
...
function(iPlayer, iCityX, iCityY)
	...
	local pPlot = Map.GetPlot(iCityX, iCityY);
	local pCity = pPlot:GetPlotCity();
	if (isCulDiv == true) then			
		if (pPlayer:IsAlive()) then
			if (pPlayer:GetCivilizationType() == GameInfoTypes["CIVILIZATION_SIOUX_MOD"]) then
				if pCity:IsCapital() then
					local cplotX = pCity:GetX();
					local cplotY = pCity:GetY();
					for iDX = -3, 3 do
						for iDY = -3, 3 do
							local pTargetPlot = Map.PlotXYWithRangeCheck(cplotX, cplotY, iDX, iDY, 3)
							if pTargetPlot then
...
for pPlayer=0, GameDefines.MAX_MAJOR_CIVS-1 do
	...
	if pPlayer:IsEverAlive() then
		if (pPlayer:GetCivilizationType() == GameInfoTypes.CIVILIZATION_SIOUX_MOD) then	
			...
			if Lang == "en_US" then
				local tquery = {"UPDATE Language_en_US SET Text = 'Wild Bison may not be improved but instead provide a large base yield and periodically roam throughout Sioux territory. Every ".. BuffTurn .."th turn the Wild Bison Resource will move to a random adjacent tile. It may move to any tile provided it is flatland, has no existing resource, and is not foreign territory. If the Wild Bison moves into neutral territory, it will claim that tile for the Sioux. However, if the Wild Bison ends up in foreign territory (due to a loss of land or City capture) the resource will be destroyed.[NEWLINE][NEWLINE]+1 [ICON_GOLD] Gold +1 [ICON_CULTURE] Culture after Chivalry is Researched.' WHERE Tag = 'TXT_KEY_RESOURCE_SIOUX_BISON_MOD_HELP'"}
